...
def calculate_pssm_thresholds(meme_path, cutoffs_path, faa_path, output_path, done_path, argv='no argv',
    ...
    if not os.path.exists(output_path):
        ...
        cmd = f'{pssm_score_peptide} -pssm {meme_path} -pssm_cutoffs {cutoffs_path} -seq {faa_path} ' \
              f'-out {output_path} -NrandPSSM {100} -CalcPSSM_Pval'
        logger.info(f'{datetime.datetime.now()}: starting CalcPSSM_Pval. Executed command is:\n{cmd}')
        subprocess.run(cmd, shell=True)
    else:
        logger.info(f'{datetime.datetime.now()}: skipping Pvalues calculation as it is already exist at:\n{output_path}')
