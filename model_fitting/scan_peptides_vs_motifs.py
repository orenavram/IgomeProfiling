...
def calculate_pssm_thresholds(meme_path, cutoffs_path, faa_path, number_of_random_pssms, 
                              shuffles, shuffles_percent, shuffles_digits, output_path, done_path, rank_method, argv='no_argv',
    ...
    if not os.path.exists(output_path):
        ...
        if rank_method == 'pval':
            ...
            cmd = f'./hits_cpp/hits -m {meme_path} -c {cutoffs_path} -s {faa_path} -o {output_path} --shuffles {shuffles} --shufflesPercent {shuffles_percent} --shufflesDigits {shuffles_digits}'
